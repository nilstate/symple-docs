/**
 * Language icons for the code sample switcher.
 *
 * DevIcon plain variants (MIT). Designed for monochrome rendering at small sizes.
 * https://devicon.dev/
 */
// DevIcon path data — viewBox and single-path d attribute
const DEVICONS = {
    bash: {
        viewBox: "0 0 128 128",
...
// Heroicons terminal (for curl)
const TERMINAL_SVG = `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m6.75 7.5 3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0 0 21 18V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v12a2.25 2.25 0 0 0 2.25 2.25Z"/></svg>`;
const CODE_SVG = `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5"/></svg>`;
const ALIASES = {
    bash: "curl", sh: "curl", shell: "curl", zsh: "curl",
    js: "javascript", jsx: "javascript",
    ts: "typescript", tsx: "typescript",
    py: "python", rb: "ruby", rs: "rust", cs: "csharp",
};
/**
 * Render a language icon as HTML.
 * curl uses a stroked Heroicon. All others use DevIcon via data-lang + CSS mask.
 */
export function langIcon(lang) {
    const raw = lang.toLowerCase();
    const key = ALIASES[raw] ?? raw;
    if (key === "curl") {
        return TERMINAL_SVG.replace("<svg ", '<svg class="lang-icon" ');
    }
    if (DEVICONS[key]) {
        return `<span class="lang-icon" data-lang="${key}"></span>`;
    }
    return CODE_SVG.replace("<svg ", '<svg class="lang-icon" ');
}
/**
 * Generate CSS rules for all language icon masks.
 * Call once at render time and inject into a <style> tag.
 */
export function langIconCSS() {
    const rules = [];
    for (const [lang, { viewBox, d }] of Object.entries(DEVICONS)) {
        const svg = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='${viewBox}'><path fill='black' d='${d}'/></svg>`;
        const encoded = encodeURIComponent(svg).replace(/'/g, '%27');
        rules.push(`#sourcey .lang-icon[data-lang="${lang}"]{-webkit-mask-image:url("data:image/svg+xml,${encoded}");mask-image:url("data:image/svg+xml,${encoded}")}`);
    }
    return rules.join("\n");
}
