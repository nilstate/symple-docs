...
const generators = {
    ...
    javascript({ url, method, contentType, body }) {
        ...
        if (body)
            lines.push(`  body: JSON.stringify(${indent(body, 4)}),`);
        lines.push(`});`, ``, `const data = await response.json();`);
        return { lang: "javascript", label: "JavaScript", source: lines.join("\n") };
    },
    typescript({ url, method, contentType, body }) {
        const lines = [`const response = await fetch('${url}', {`];
        lines.push(`  method: '${method}',`);
        if (contentType) {
            lines.push(`  headers: {`, `    'Content-Type': '${contentType}',`, `  },`);
    ...
    python({ url, method, contentType, body }) {
    ...
    ruby({ url, method, contentType, body }) {
        const lines = [
            `require 'net/http'`,
            `require 'json'`,
            ``,
 ...
 */
export function generateCodeSamples(op, serverUrl, langs = DEFAULT_CODE_SAMPLE_LANGS) {
    // Spec-defined samples always win
    if (op.codeSamples?.length)
        return op.codeSamples;
...
function getRequestBodyExample(op) {
    ...
    if (mediaType.schema)
        return generateExample(mediaType.schema);
    return undefined;
}
function pythonDict(jsonStr) {
    return jsonStr
        .replace(/: true/g, ": True")
