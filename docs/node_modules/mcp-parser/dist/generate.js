 ...
 * ```
 */
export function generateLlmsTxt(spec, baseUrl) {
    const lines = [];
 ...
 * @param spec - The MCP spec to generate from
 */
export function generateLlmsFullTxt(spec) {
    const lines = [];
    lines.push(`# ${spec.server.name}`);
 ...
 * @param spec - The MCP spec to generate from
 */
export function generateMarkdown(spec) {
...
function formatTool(tool) {
    ...
    if (tool.annotations) {
        ...
        if (tool.annotations.idempotentHint)
            badges.push("idempotent");
        if (tool.annotations.openWorldHint)
            badges.push("open-world");
        if (badges.length) {
...
function formatResourceTemplate(template) {
    const lines = [];
